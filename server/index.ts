...
app.post('/api/add-media', authenticateToken, async (req: Request, res: Response) => {
    ...
    try {
        ...
        const existingRows = existingMoviesResponse.data.values?.slice(1) || [];
        const existingIndex = existingRows.findIndex(row => row[0].toLowerCase() === officialName.toLowerCase());
        const existingMovieNames = new Set(existingRows.map(row => row[0].toLowerCase()));
        ...
        if (existingIndex !== -1) {
            ...
            const colLetter = String.fromCharCode('A'.charCodeAt(0) + sheetConfig.columns.length - 1);
            await sheets.spreadsheets.values.update({
                spreadsheetId: SPREADSHEET_ID,
                range: `${sheetConfig.sheetName}!A${rowIndex}:${colLetter}${rowIndex}`,
